...
def test_normalizer():
    ...
    assert torch.allclose(mean, torch.mean(
        x, dim=(0, 1, 2))), "Mean is not correct"
    assert torch.allclose(std, torch.std(x, dim=(0, 1, 2)),
                          rtol=1e-3), "Standard deviation is not correct"
...
def test_normalizer_save():
    ...
    normalizer.save_stats(
        "tests/test_model/test_jukebox_normalize.pt", (mean, std))
    ...
    assert torch.allclose(normalizer.std, std,
                          rtol=1e-3), "Standard deviation is not correct"
...
def test_normalizer_normalize_denormalize():
    ...
    assert torch.allclose(x_norm_mean, torch.zeros_like(
        x_norm_mean)), "Mean is not correct"
    assert torch.allclose(x_norm_std, torch.ones_like(
        x_norm_std), rtol=1e-3), "Standard deviation is not correct"
...
def test_normalize_cuda():
    normalizer = JukeboxNormalizer(
        "config/normalizations/maestro_val_lvl_1.pt").to("cuda")
    ...
    assert torch.allclose(x, x_denorm, rtol=1e-3,
                          atol=1e-5), "Normalization is not invertible"
